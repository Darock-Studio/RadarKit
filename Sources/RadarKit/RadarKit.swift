...
public struct RKFeedbackLink: View {
    ...
    public init(projectName: String, showProjectName: String, gatherAppdiagnose: Bool = true, showUnderstandStatus: Bool = true, checkUpdate: RKUpdateCheckMethod = .none, showsReplyBadge: Bool = true) {
        ...
        self.showsReplyBadge = showsReplyBadge
    }
...
/// 使用此枚举以定义是否需要检查更新以及检查方式。
public enum RKUpdateCheckMethod {
    /// 不检查更新。
    case none
    
    /// 通过 RadarKit 检查更新，传入获取当前最新版本的链接。
    ///
    /// 访问链接应当响应像这样使用点将数字分割为三部分的字符串：
    /// 1.0.0
    /// 一些总是附带引号返回字符串的服务器也可以使用：
    /// "1.0.0"
    case auto(String)
    
    /// 手动检查更新，传入当前是否有可用更新。
    case manual(Binding<Bool>)
}

public struct RKFeedbackView: View {
    var projName: String
    var gatherAppdiagnose: Bool
    var showUnderstandStatus: Bool
    @Environment(\.FAQView) var faqView
    ...
    public init(projName: String, gatherAppdiagnose: Bool = true, showUnderstandStatus: Bool = true) {
    ...
    struct NewFeedbackView: View {
        ...
        @Environment(\.FeedbackTitleSample) var titleSample
        @Environment(\.FeedbackAttachmentSelector) var attachmentSelector
        @Environment(\.FeedbackExtData) var extData
        @Environment(\.Tipper) var tipper
        @Environment(\.NewFeedbackAction) var newFeedbackAction
        ...
        var body: some View {
            Form {
                List {
                    ...
                    if gatherAppdiagnose || attachmentSelector != nil {
                        Section {
                            if !dontSendDiagnose {
                            ...
                            if !attachments.isEmpty {
                                ForEach(0..<attachments.count, id: \.self) { i in
                    ...
                    Section {
                        Button(action: {
                            if titleInput == "" {
                                tipper?("标题不能为空", "xmark.circle.fill")
                                return
                            }
                            isSending = true
                            let extDiags = { () -> String in
                                if _fastPath(!dontSendDiagnose) {
                                    ...
                                    OS：\(systemVersion)\({ () -> String in
                                    guard !extData.isEmpty else { return "" }
                                    var tmp = "\n"
                                    for (key, value) in extData {
                                    tmp += "\(key)：\(value)\n"
                                    }
                                    tmp.removeLast()
                                    return tmp
                            ...
                            DarockKit.Network.shared
                                .requestString("https://fapi.darock.top:65535/feedback/submit/anony/\(projName)/\(msgToSend.base64Encoded().replacingOccurrences(of: "/", with: "{slash}"))") { respStr, isSuccess in
                                    if isSuccess {
                                        if Int(respStr) != nil {
                                            ...
                                            tipper?("已发送", "paperplane.fill")
                                            dismiss()
                                        } else {
                                            tipper?("服务器错误", "xmark.circle.fill")
                                        }
                                    }
                                }
                        }, label: {
            ...
            .sheet(isPresented: $isReplyPresented, onDismiss: {
                ...
                TextField("回复信息", text: $replyInput) {
                    ...
                    if replyInput != "" {
                        ...
                        DarockKit.Network.shared
                            .requestString("https://fapi.darock.top:65535/radar/reply/\(projName)/\(id)/\(enced)") { respStr, isSuccess in
                                if isSuccess {
                    ...
                    if !(!from[i].contains("：") && !from[i].contains(":") && i == 0) && (!from[i].hasPrefix("Sender")) && (!from[i].hasPrefix("Time")) {
                        ...
                        HStack {
                            if (from[i].contains("：") && from[i] != "：" ? from[i].split(separator: "：")[0] : "") == "State" {
                                ...
                                Text(from[i].split(separator: "：").count > 1 ? String(from[i].split(separator: "：", maxSplits: 1)[1]).dropLast("\\") : from[i].dropLast("\\"))
                                    .font(.system(size: 14))
                            }
                            Spacer()
                        }
